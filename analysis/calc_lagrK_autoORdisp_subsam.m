...
[um_t,u_t,ue_t] = deal(zeros(grid_h.niu,grid_h.nju,ntuv));
[vm_t,v_t,ve_t] = deal(zeros(grid_h.niv,grid_h.njv,ntuv));
...
for it = 1:ntuv
    ...
    um_t(:,:,it) = um(:,:,ik) * factor; % [km/d]
    vm_t(:,:,it) = vm(:,:,ik) * factor;
    ...
    if wichmethd == 2 % disp
        ...
        ue_t(:,:,it) = u_t(:,:,it) - um_t(:,:,it);
        ve_t(:,:,it) = v_t(:,:,it) - vm_t(:,:,it);
    end
end
...
%         mlen = Ltraj;
        ...
        if wichmethd == 1
            ...
            for it = 1:ntuv
                ulm(:,it) = interp2(xu,yu,um_t(:,:,it)',xtr(:,it),ytr(:,it),'cubic'); % km/d
