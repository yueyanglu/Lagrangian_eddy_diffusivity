% 
% Calculate the coarse-grid uhc by subsampling the pre-calc trajs and Lagr velocities.
...
carry_al = 1:8;
ntr = numel(carry_al);
expStr = '10'; % corresponds to subsampling ratio: 1, 0.5, 0.125...
...
if tloop == 1
    ...
    elseif useWichInit == 2
        h0_fnm = [exp_dir '/h/lp' num2str(tloop-1,'%02d') '/Z' num2str(ik,'%02d') ...
...
%     ulvl_fnm = traj_fnm;
    ...
    for iu = 2:grid.niu-1
        for ju = 1:grid.nju
            ...
            if isempty(indx_temp)
                ...
                h_ptcls = hPerPtcl0_1d(indx_temp);
    ...
    for itr = 1:ntr
        uhc_bin{itr} = fillmissing2(uhc_bin{itr},'linear');
        vhc_bin{itr} = fillmissing2(vhc_bin{itr},'linear');
    end
    ...
    for itr = 1:ntr
        ...
        global_att  = [ 'Euelerian <uhc> reconstructed from SUBSAMPLED particles; ' ...
