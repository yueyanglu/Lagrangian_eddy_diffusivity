...
trajs_orig_dir = [exp_dir '/trajs/lp' num2str(tloop,'%02d') '/'];
ulvl_orig_dir = [exp_dir '/ulvl/lp' num2str(tloop,'%02d') '/'];
...
% 
% -- initial positions
params_fnm = [exp_dir 'trajs/lp' num2str(tloop,'%02d') '/params.mat'];
ds_params = load(params_fnm);
xin1d = ds_params.xin1d;
yin1d = ds_params.yin1d;

%----- assign each ptcl with h/c 
...
%     ulvl_fnm = traj_fnm;
    ...
    for itr = 1:ntr
        ...
        global_att  = [ '<uc> reconstructed from particles; ' ...
