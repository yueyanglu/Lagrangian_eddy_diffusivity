...
trajs_orig_dir = [campdir '/lagr_study/exp1/trajs_cubic/lp' num2str(tloop,'%02d') '/Z' num2str(ik,'%02d') '/full'];

% ------- save dir
exp_dir = [campdir '/lagr_study/exp1/'];
if ~exist(exp_dir,'dir');  mkdir(exp_dir); end
save_np_dir = [exp_dir '/nptcls_cubic/lp' num2str(tloop,'%02d') '/Z' num2str(ik,'%02d')];
save_h_dir = [exp_dir '/h_cubic/lp' num2str(tloop,'%02d') '/Z' num2str(ik,'%02d')];
save_c_dir = [exp_dir '/c_cubic/lp' num2str(tloop,'%02d') '/Z' num2str(ik,'%02d')];
...
% -- initial positions
% params_fnm = [exp_dir 'trajs_cubic/lp' num2str(tloop,'%02d') '/params.mat'];
params_fnm = [exp_dir 'trajs_cubic/lp' num2str(tloop,'%02d') '/Z01/full/params.mat'];
ds_params = load(params_fnm);
xin1d = reshape(ds_params.xin2d, [numel(ds_params.xin2d) 1]);
yin1d = reshape(ds_params.yin2d, [numel(ds_params.yin2d) 1]);
...
volPerPtcl0_1d = perBin2perPtcl_gen(volPerBin./1024 ...
...
%         fprintf('Doing bin i = %d from nxbins=%d \n', ibin, nxbins)
        for jbin = 1:nybins
            ...
            if isempty(indx_temp)
                ...
                hPerBin(ibin,jbin) = mean(hPerPtcl_1d(indx_temp),'omitnan');
                % vol of bin = sum of vol carried by ptcls
                volPerBin(ibin,jbin) = sum(volPerPtcl_1d(indx_temp),'omitnan');
                ...
                for itr = 1:ntr
                    ...
                    %trPerBin{itr}(ibin,jbin) = mean(trPerPtcl_1d{itr}(indx_temp),'omitnan');
                end
            end
    ...
    data = {numPerBin};
    dimNum_of_var = {[1,2]};
    global_att  = ['Euelerian num of ptcls reconstructed from particles; '];
    FUN_nc_easywrite_enhanced( save_np_fnm, dim_name, dim_length,...
        varname, dimNum_of_var, data, global_att )
    fprintf(1,'Num of ptcls saved to: %s...\n', save_np_fnm);
    ...
    global_att  = [ 'Euelerian h, vol reconstructed from particles; ' ...
        '; h0_fnm=' h0_fnm];
